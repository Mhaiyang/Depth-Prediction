...
def depth_loss_graph(input_gt_depth, predict_depth)
    ...
    loss = Kloss.mean_squared_error(target_depth, predict_depth)
...
def data_generator(dataset, config, shuffle=True, augment=False, batch_size=1):
    ...
    while True:
        try:
            ...
            image, depth = load_image_gt(dataset, image_id, augment=augment)
            ...
            if b == 0:
                ...
                batch_depth = np.zeros(
                    (batch_size, depth.shape[0], depth.shape[1]), dtype=np.float32)

            # Add to batch
            batch_images[b] = mold_image(image.astype(np.float32), config)
            batch_depth[b] = depth.astype(np.float32)

            b += 1

            # Batch full?
            if b >= batch_size:
                inputs = [batch_images, batch_depth]
                outputs = []
...
############################################################
#  Sub-pixel implemented by Keras
############################################################
class SubpixelConv2D(KE.Layer):

    def __init__(self, channel, scale=2, **kwargs):
        super(SubpixelConv2D, self).__init__(**kwargs)
        self.input_channel = channel[0]
        self.output_channel = channel[1]
        self.scale = scale

    def call(self, inputs, **kwargs):

        output = tf.depth_to_space(inputs, self.scale)
...
class FCRN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_depth = KL.Input(shape=[640, 480], name="input_gt_depth", dtype=tf.float32)

        # Build the backbone layers.
        C5 = ResNet(input_image, layers=101)

        x = KL.Conv2D(1024, (3, 3), strides=(1, 1), padding="same", name="fcrn_conv0", activation="relu")(C5)

        x = SubpixelConv2D([1024, 256], 2, name="fcrn_up1")(x)
        x = KL.Conv2D(256, (3, 3), strides=(1, 1), padding="same", name="fcrn_conv1", activation="relu")(x)

        x = SubpixelConv2D([256, 64], 2, name="fcrn_up2")(x)
        x = KL.Conv2D(64, (3, 3), strides=(1, 1), padding="same", name="fcrn_conv2", activation="relu")(x)
        ...
        if mode == "training":
            ...
            outputs = [predict_depth, depth_loss]
            model = KM.Model(inputs, outputs, name='FCRN')
